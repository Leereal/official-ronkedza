...
const populateSocialPlatform = (query) => {
...
export const getPlatform = async (name) => {
  try {
    await connectToDatabase();
    const platform = await SocialPlatform.findOne({ slug: name });
    return platform;
  } catch (error) {
    handleError(error);
    return null;
...
export async function getAllSocialPlatforms({ query, limit = 100, page }) {
  try {
    ...
    const socialPlatformsQuery = await SocialPlatform.find(conditions)
      .sort({ createdAt: "desc" })
      .skip(skipAmount)
      .limit(limit);

    return {
      data: JSON.parse(JSON.stringify(socialPlatformsQuery)),
    };
  } catch (error) {
    handleError(error);
  }
