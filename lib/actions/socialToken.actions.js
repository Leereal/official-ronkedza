...
const exchangeUserTokenForPageTokens = async (
  ...
  if (app) {
    try {
      ...
      console.log("pageTokens", accountsData);
...
export async function createSocialAuth({ userId, data, socialPlatform }) {
  try {
    ...
    } else if (socialPlatform.slug === "instagram-business") {
      try {
        if (data.socialId) {
          //Get page data
          const socialToken = await getSocialTokenById(data.socialId);
          const pageId = socialToken?.socialId;

          // Construct the Instagram Business API endpoint
          const instagramBusinessEndpoint = `${socialPlatform.endpoint}/${pageId}?fields=instagram_business_account&access_token=${user.facebookToken.accessToken}`;
          console.log(
            "instagramBusinessEndpoint : ",
            instagramBusinessEndpoint
          );
          // Make a request to the Instagram Business API endpoint
          const response = await fetch(instagramBusinessEndpoint);
          const responseData = await response.json();
          console.log("Response Data : ", responseData);

          // Extract relevant data from the response
          const account = responseData?.instagram_business_account;

          //Use account id to get instagram business data
          const instagramInfoURL = `${socialPlatform.endpoint}/${account.id}?access_token=${user.facebookToken.accessToken}`;

          // Make a request to the Instagram Business API endpoint
          const responseInfo = await fetch(instagramInfoURL);
          const infoData = await response.json();
          console.log("Insta Info : ", infoData);
          // const instaData = {
          //   socialId: account?.id,
          //   name: meUser?.data?.name,
          //   link: `@${meUser?.data?.username}`,
          //   accessToken: user.facebookToken.accessToken,
          //   followers_count: meUser?.data?.public_metrics.followers_count,
          // };

          // Validate if the required data is present
          // if (instagramBusinessData) {
          //   socials = await addSocialToken(userData, userId, socialPlatform);
          //   const socials = await saveInstagramBusinessData(instagramBusinessData, userId, socialPlatform);
          // } else {
          //   throw new Error("Invalid Instagram Business data received");
          // }
        } else {
          throw new Error("Missing data, please provide all fields");
        }
      } catch (error) {
        handleError(error);
      }
