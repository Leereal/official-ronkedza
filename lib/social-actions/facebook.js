...
const pageId = process.env.NEXT_PUBLIC_FACEBOOK_APP_ID;
const pageToken = process.env.FACEBOOK_PAGE_TOKEN;
const FB_ENDPOINT = process.env.GRAPH_API_ENDPOINT;

const socialPlatform = SocialPlatform.findOne({ name: "Facebook" });
...
export const createPagePost = async (data) => {
  try {
    ...
    if (
      ...
      facebookTokens.facebookUserId &&
      currentTime < facebookTokens.expiration
    ) {
      let apiUrl;
      let postData = {
...
const saveResponse = async (data, response) => {
  try {
    await connectToDatabase();
