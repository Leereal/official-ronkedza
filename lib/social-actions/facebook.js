...
export const createPagePost = async (data, facebookTokens) => {
  //Social tokens contains socialPlatform from them then use them
  const socialPlatform = facebookTokens.socialPlatform;
  try {
    ...
    if (
      ...
      let apiUrl;
      const pageId = facebookTokens.socialId;
      ...
      if (data.attachments && data.attachments.length) {
        ...
        const uploadedPhotoIds = await Promise.all(
          data.attachments.map(async (image) => {
      ...
      console.log("Done Posting : ", responseData);
      //Save the response
      createPostResult(data._id, responseData, facebookTokens._id);
    } else {
...
export const createGroupPost = async (data, facebookTokens) => {};
